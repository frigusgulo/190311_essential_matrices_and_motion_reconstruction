...
class camClass(Image):
	...
	def ptCloud(self):
		...
		I1 = self.images[0].array
		I2 = self.images[1].array
		h,w,d = I1.shape
		...
		cv = self.images[0].h/2
		cu = self.images[0].w/2

		K_cam = np.array([[self.images[0].f,0,cu],[0,self.images[0].f,cv],[0,0,1]])
